...
exports.collection = function(originalCollection, n, days){
    ...
    var daylist = daysToInterpolate.map(function(day) {
      var date = timeStart.advance(day, 'day').format("YYYY-MM-dd");
      return date;
    });
    var datefilter = ee.Filter.inList({
        leftField: 'date',
        rightValue: daylist,
    });
    ...
    var initImages = daysToInterpolate.map(function(day) {
      var image = initImage.set({
        ...
        'date': timeStart.advance(day, 'day').format("YYYY-MM-dd"),
    ...
    originalCollection = originalCollection.map(function(image) {
      ...
      return image.addBands(timeImageMasked).toFloat()
                  .copyProperties(image, ['system:index',"system:time_start",'system:id',
                                          'date','type']);
    });
    ...
    var regularCol = interpolatedCol//.filter(datefilter)
                        .filter(ee.Filter.eq('type', 'interpolated'));
